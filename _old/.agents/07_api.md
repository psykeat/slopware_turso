...
- **Code-first registry.** Capabilities are TypeScript values defined next to the services they wrap. The `entity_commands` table is a _projection_ of the registry, never a second source of truth.
- **Registry-backed entity knowledge.** Domain-level entity metadata lives in `@repo/registry` and is exposed through `system.registry.*` read capabilities. Raw schema introspection remains a compatibility source, not the preferred place for new domain semantics.
- **One validation point.** Input is validated exactly once, in `executeCapability`, with Zod schemas. Services that self-validate (e.g. variant templates) share the _same imported schema_ — schemas are never duplicated.
...
packages/db/src/capabilities/
  ...
  modules/system.registry.ts               exposes @repo/registry metadata capabilities
...
packages/db/src/persistence/postgres.ts         current Postgres compatibility runtime
packages/db/src/turso/routing.ts               target Turso tenant-route contract
```
...
**Tenant scope**: the capability runtime calls `runInTenantScope(ctx, handler)`.
The current Postgres compatibility adapter implements this with a
transaction-local tenant GUC for RLS. The Turso adapter must implement the same
interface by resolving the server-side tenant route and selecting the physical
tenant database before handler execution.

**Transactions**: ordinary capability handlers should not open wrapper
transactions in the capability layer. `DataService`/`DocumentService` own
operation-level transactions internally. An operation that needs multi-step
atomicity must become its own capability backed by one service method.
...
- `system.registry.*`: `discoverEntities`, `resolveProjection`, `validatePayload`, `generateFixture`, `explainConstraint`
- `system.organization.*`
- `system.tenant.*`
- `system.company.*`
- `system.userTenant.*`
