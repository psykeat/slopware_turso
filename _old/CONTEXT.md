...
tool- and output-architecture. The overlay gives the user _and_ the model a stable
...
- **Context Picker** — The overlay control by which the _user_ chooses the active
...
- **Approval Flow** — The interactive human-in-the-loop path that lets a confirm-gated
  capability run from chat. (The current `/api/ai/execute` route _excludes_ confirm
...
- **Exploration / Proposal / Confirmation** — The three conversation states of the
  overlay. _Exploration_: the model reads more context, extracts hints, surfaces
  candidate matches. _Proposal_: the model emits a typed proposal card (e.g. likely
  customer, quote number, next action). _Confirmation_: only once the assignment and
...
- **Confirm-gates are an execution boundary, not a thinking boundary.** The model may
  ...
  the model from _reasoning further_.
...
- **Maximum context evaluation before any hard stop.** No early abort on ambiguous
  ...
  body). "Too uncertain" is a conclusion reached _after_ context expansion and
...
- **Capabilities are verbs, not scenarios.** A capability is named after its
  ...
  scenario (`confirmQuoteFromMail`). Scenarios are assembled _at runtime_ by the
...
- **Decision Card** — A structured disambiguation surface (a Proposal-Card variant in
  ...
  **last** resort, reached only _after_ exhausted context evaluation — never a pre-wired
  gate. Free-text steering stays available as a secondary channel, not the primary form.
  Distinct from Approval: Approval guards a _write_; a Decision resolves an _ambiguity_.
...
- **Proposal Card** — A single typed, editable structured-output view of an intended
  ...
  visibly _revised_ when reality diverges (e.g. the quote was already converted)
