...
references, then chain atomic write-verbs. Transactional invariants live _inside_ a
single capability (`convert` is atomic); orchestration _across_ capabilities is the
...
executor, and that some genuinely composite needs surface as _missing atomic verbs_ that
