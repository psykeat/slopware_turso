...
import type { GridSort, GridState, FilterRule } from "@repo/ui/hooks/use-grid-state";

function pk(prefix: string, key: string) {
  return prefix ? `${prefix}_${key}` : key;
}

export function useGridUrlState(options?: { defaultPageSize?: number; prefix?: string }): GridState {
  const prefix = options?.prefix ?? "";
  const defaultSize = options?.defaultPageSize ?? 50;

  const readFromUrl = (): {
    page: number; pageSize: number; sort: GridSort | null;
    search: string; filters: FilterRule[];
  } => {
    if (typeof window === "undefined") return { page: 1, pageSize: defaultSize, sort: null, search: "", filters: [] };
    ...
    const sort: GridSort | null = sortStr
      ? (() => { const [key, dir] = sortStr.split(":"); return key ? { key, dir: (dir === "desc" ? "desc" : "asc") as "asc" | "desc" } : null; })()
      : null;
    const search = p.get(pk(prefix, "q")) ?? "";
    const filtersStr = p.get(pk(prefix, "f"));
    const filters: FilterRule[] = filtersStr ? (() => { try { return JSON.parse(filtersStr); } catch { return []; } })() : [];
  ...
  useEffect(() => {
    const p = new URLSearchParams(window.location.search);
    const set = (k: string, v: string | undefined) => v ? p.set(pk(prefix, k), v) : p.delete(pk(prefix, k));
  ...
  const setPageSize = (s: number) => { setPageSize_(s); setPage_(1); };
  const setSort = (s: GridSort | null) => { setSort_(s); setPage_(1); };
  const setSearch = (s: string) => { setSearch_(s); setPage_(1); };
  const setFilters = (f: FilterRule[]) => { setFilters_(f); setPage_(1); };
  ...
  return { page, pageSize, sort, search, filters, setPage, setPageSize, setSort, setSearch, setFilters, queryParams };
}
