...
async function loadTenantLlmConfig(
  ...
  const [row] = await db
    ...
    .where(eq(tenantLlmConfig.companyId, companyId))
    .limit(1);
        ...
        return runInTenantScope(context, async () => {
          try {
            if (path === "/api/ai/llm-config") {
              ...
              return new Response(
                JSON.stringify(
                  globalConfig ? maskLoadedConfig(globalConfig) : { configured: false },
                ),
                { headers: { "content-type": "application/json" } },
              );
            }
            ...
            if (path === "/api/ai/catalog/entities") {
              ...
              const catalog = await AIDiscoveryService.getSemanticEntityCatalog(
                tenantId,
                taskScope,
              );
            ...
            if (path === "/api/ai/memories") {
              const memories = await db.select().from(aiMemory).orderBy(desc(aiMemory.createdAt));
                ...
                async start(controller) {
                  ...
                  try {
                    const [sessionRow] = await db
                      .select()
                      .from(aiSession)
                      .where(eq(aiSession.sessionId, sessionId))
                      .limit(1);
            ...
            return new Response(
              JSON.stringify({ error: e.message, errorClass: "AI_UNAVAILABLE" }),
              {
                status: 500,
                headers: { "content-type": "application/json" },
              },
          ...
          try {
            if (path === "/api/ai/llm-config") {
              ...
              if (scope === "tenant") {
                ...
                const [companyRow] = await db
                  ...
                  .from(company)
                  .where(eq(company.companyId, body.companyId))
                  .limit(1);
                ...
                const [existing] = await db
                  .select()
                  .from(tenantLlmConfig)
                  .where(eq(tenantLlmConfig.companyId, body.companyId))
                  .limit(1);
              ...
              const [existing] = await db
                ...
                .from(systemSettings)
                .where(
                  and(eq(systemSettings.scope, "global"), eq(systemSettings.key, "llm_config")),
                )
                .limit(1);
            ...
            if (path === "/api/ai/sessions") {
              ...
              if (!body?.focusType || !body.focusId) {
                return new Response(
                  JSON.stringify({ error: "focusType and focusId are required" }),
                  {
                    status: 400,
                    headers: { "content-type": "application/json" },
                  },
              ...
              if (activePersistence.provider === "postgres") {
                insertValues.tenantId = tenantId;
              }

              const [sessionRow] = await db.insert(aiSession).values(insertValues).returning({
            ...
            if (path === "/api/ai/variant-template-suggest") {
              ...
              if (body.articleId) {
                ...
                const [articleRow] = await db
                  ...
                  .where(eq(article.articleId, body.articleId))
                  .limit(1);
            ...
            if (path.startsWith("/api/ai/memories/") && path.endsWith("/confirm")) {
              ...
              const [updated] = await db
                ...
                .where(eq(aiMemory.memoryId, memoryId))
                .returning();
            ...
            if (path.startsWith("/api/ai/memories/") && path.endsWith("/reject")) {
              ...
              await db.delete(aiMemory).where(eq(aiMemory.memoryId, memoryId));
