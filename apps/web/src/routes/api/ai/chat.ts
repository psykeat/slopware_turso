...
// excludes confirm actions because it has no human-in-the-loop), this route
...
// lives in the action execution log and replay is safe (idempotency tests).
//
// Tool selection follows "reads global, writes scoped": every read action is
...
const SYSTEM_PROMPT = `You are the in-app assistant for an ERP system, operating over a registry action toolset.
...
- Chain atomic verbs to accomplish a goal; scenarios are not pre-wired. Every data-changing action requires explicit user approval at its own write-boundary — request it, do not assume it.
