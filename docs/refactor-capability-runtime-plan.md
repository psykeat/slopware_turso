...
export function createCapabilityTools(ctx: ExecutionContext) {
  ...
  for (const capability of capabilities) {
    ...
    tools[toolName] = toolDefinition({
      name: toolName,
      description: `${capability.summary.de} / ${capability.description?.de || ""}. \n` +
...
- **Body**:
  ...
  ```
  *(Hinweis: `confirmMode: "exclude"` ist Standard, da hier kein interaktiver Handshake stattfindet. Nutze `"allow"`, wenn du die Ausführung schreibender Confirm-Capabilities explizit erlauben möchtest.)*
- **Response**:
  ```json
  {
...
Denkgrenze**. Das Overlay ist ein *iteratives Klärungswerkzeug*: lesen → bei
...
- **Abgrenzung zu Approval:** *Approval* (`needsApproval`, server-seitiges Tool) bewacht
  einen **Write** — Ja/Nein vor Ausführung. *Decision* (Client-Tool, kein Write) löst eine
