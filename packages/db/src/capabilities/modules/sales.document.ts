...
const draftLineInputSchema = z.object({
  ...
  taxReason: z.string().nullable().optional(),
  taxRuleId: z.uuid().nullable().optional(),
  taxCountryCodeUsed: z.string().nullable().optional(),
  taxRateSnapshot: z.union([z.string(), z.number()]).nullable().optional(),
...
export const documentPricing = defineCapability({
  ...
  exposure: {
    ...
    ai: {
      ...
      resultShape: "{ unitPrice, taxCodeId, taxReason, taxRuleId, taxCountryCodeUsed, taxRate }",
    },
  },
  schemaVersion: 1,
  handler: async (ctx, input) =>
