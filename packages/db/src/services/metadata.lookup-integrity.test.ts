...
import { entityCapabilityManifest } from "../capabilities/manifest.generated";
import { closeDb, db } from "../index";
import { helperTableRegistry, tenantFields } from "../schema/app.schema";
import { discoverTables, resolveExplicitLookupTable, resolveLookupMetadata, resolveLookupTable } from "./metadata";
...
function columnFamily(columnType: string | undefined): ColumnFamily {
  ...
  if (columnType === "PgVarchar" || columnType === "PgChar" || columnType === "PgText") return "character";
  return "other";
}

test("every curated lookup column points at an existing, type-compatible list capability", async () => {
  ...
  const tenantOverrides = await db.select().from(tenantFields).where(eq(tenantFields.scope, "tenant"));
  ...
  for (const { key: entityName, table } of tables) {
    ...
    for (const [colName, col] of Object.entries(columns)) {
      ...
      const lookupTable =
        tenantOverrideMap.get(`${entityName}.${colName}`) ?? resolveLookupTable(entityName, colName);
      ...
      const listOp = entityCapabilityManifest[lookupTable]?.ops?.list;
      if (isCurated) {
        assert.ok(
          listOp,
          `${entityName}.${colName} is a curated lookup -> "${lookupTable}", which has no list capability`,
        );
      } else if (!listOp) {
      ...
      const valueCol = meta.inferredValueColumn ? targetColumns[meta.inferredValueColumn] : undefined;
