...
export function CustomerStatsSection({ addressId }: { addressId: string }) {
  ...
  const colHeaderClassRight =
    "text-[11px] uppercase tracking-wider font-medium text-ink-mute text-right select-none px-3 py-0";
  const rowClass =
    "h-10 border-b border-hairline last:border-b-0 hover:bg-canvas transition-colors";
        ...
        <table className="w-full table-fixed border-collapse">
          <thead>
            <tr className="h-8 border-b border-hairline">
              ...
              <th className={colHeaderClass} style={{ width: "22%" }}>
                Belegnummer
              </th>
              <th className={colHeaderClass} style={{ width: "8%" }}>
                Typ
              </th>
              <th className={colHeaderClass} style={{ width: "16%" }}>
                Status
              </th>
              <th className={colHeaderClassRight} style={{ width: "20%" }}>
                Brutto
              </th>
            </tr>
          </thead>
  ...
  return (
    <div className="flex flex-col gap-4">
        ...
        <tbody>
          {documents.map((doc) => (
            <tr key={doc.documentId} className={rowClass}>
              <td className={`${cellClass} tabular-nums`}>{formatDate(doc.documentDate)}</td>
              <td className={`${cellClass} font-mono`}>{doc.documentNo}</td>
              <td className={`${cellClass} font-mono`}>{doc.documentType}</td>
              <td className={cellClass}>
                <StatusDot status={doc.status ?? "draft"} />
              </td>
              <td className={cellClassRight}>{formatMoney(doc.totalGross ?? 0)}</td>
            </tr>
          ))}
        </tbody>
      </table>
      ...
      {annualRevenue.length > 0 && (
        <div>
          <div className="flex h-8 items-center border-b border-hairline px-3 text-[11px] font-medium tracking-wider text-ink-mute uppercase">
            Jahresumsatz
          </div>
          <table className="w-full table-fixed border-collapse">
            <thead>
              <tr className="h-8 border-b border-hairline">
                <th className={colHeaderClass} style={{ width: "30%" }}>
                  Jahr
                </th>
                <th className={colHeaderClassRight} style={{ width: "40%" }}>
                  Umsatz
                </th>
              </tr>
            </thead>
            <tbody>
              {annualRevenue.map(({ year, total }) => (
                <tr key={year} className={rowClass}>
                  <td className={`${cellClass} font-mono tabular-nums`}>{year}</td>
